    ...
    namespace ops {
        ...
        TORCH_LIBRARY_FRAGMENT(rasa, m) {
            m.def(TORCH_SELECTIVE_SCHEMA(
                          "rasa::spherical_pad2d(Tensor input, int pad_l, int pad_r, int pad_u, int pad_d, str interpolation) -> Tensor")
            );
            m.def(TORCH_SELECTIVE_SCHEMA(
                          "rasa::_spherical_pad2d_backward(Tensor grad, Tensor input, int pad_l, int pad_r, int pad_u, int pad_d, str interpolation) -> Tensor")
            );
        }
    } // namespace ops
} // namespace rasa
