...
def main():
    ...
    x = torch.rand(batch_sz, channels, affinity_w,
    ...
    A_ref = torch.rand(channels, affinity_w, dtype=torch.float64, device=device)
    A_ref = torch.linspace(0, 1, affinity_w).view(1, -1).repeat(channels, 1).to(dtype=torch.float64, device=device)
    # plt.subplot(131)
